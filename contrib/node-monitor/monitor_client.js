const coinevonet = require('./coinevonet') // currently using the 0.5 api
const fs = require('fs')
...
// while we monitor snode and coinevo addresses
...
var coinevonet_config = {
  binary_path: '../../coinevonet',
...
var coinevonet_version = 'unknown'
...
function addSnode(snode) {
  ...
  if (idx == -1) {
    ...
    function lookupSnode(snode) {
      coinevonet.lookup(snode, function (records) {
...
coinevonet.onMessage = function (data) {
  console.log(`monitor: ${data}`)
  var lines = data.split(/\n/)
  for (var i in lines) {
    var tline = lines[i].trim()
    // 	coinevonet-0.4.0-59e6a4bc (dev build)
    if (tline.match('coinevonet-0.')) {
      var parts = tline.split('coinevonet-0.')
      coinevonet_version = parts[1]
    ...
    if (tline.match('Set Up networking for')) {
      ...
      // default:9j4uido1ai7ucirbncbqii1395e8ccd6cjomo9cccp3ztx1ukwio.coinevo
    ...
    if (tline.match(' is built, took ')) {
      // path TX=8826efd28bede66c59782af9894eed08 RX=a994770a8c6d61700b8ca65e4a3adea3 on OBContext:default:3y3ch3m6c8xgmutxwe8fger6n963hn3mkn6tk14j67w1zdxj1n1y.coinevo-icxqqcpd3sfkjbqifn53h7rmusqa1fyxwqyfrrcgkd37xcikwa7y.coinevo is built, took 2321 ms
    ...
    //TX=53e833fcbad1395647f198201d4931e5 RX=b4768bc4b91b86b51b513319f443f538 on default:dnhi78pwrn6cd5yz83i83816nqudwym57s5x4bp3j4g5kbeigw5y.coinevo built latency=291
    ...
    //service/endpoint.cpp:652	default:kzow69uftho8ukm8g4kf88y5zka84azfrmfx5okkmrg7ucdz5d1o.coinevo IntroSet publish confirmed
    if (tline.match('IntroSet publish confirmed')) {
      score_total++
    }
  }
}
coinevonet.onError = function (data) {
...
function checkIP(cb) {
  coinevonet.getCoinevoNetIP(function (ip) {
    if (ip === undefined) {
      checkIP(cb)
      return
    }
    //console.log('coinevonet interface ip', ip)
    cb(ip)
  })
}

coinevonet.startClient(coinevonet_config, function () {
  // coinevonet isn't necessarily running at this point
  console.log('Starting monitor')
  score_time = Date.now()
  checkIP(function (ip) {
    console.log('coinevonet interface ip', ip)
    coinevonet.findCoinevoNetDNS(function (servers) {
    ...
    setTimeout(function () {
      console.log(coinevonet_config.binary_path, 'version', coinevonet_version, 'final score', score_total)
      process.exit()
