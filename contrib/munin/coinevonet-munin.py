...
def exit_sessions_main():
    if len(sys.argv) == 2 and sys.argv[1] == 'config':
        print("graph_title coinevonet exit sessions")
        print("graph_vlabel sessions")
        print("graph_category network")
        print("graph_info This graph shows the number of exit sessions on a coinevonet exit")
...
def peers_main():
    if len(sys.argv) == 2 and sys.argv[1] == 'config':
        print("graph_title coinevonet peers")
        print("graph_vlabel peers")
        print("graph_category network")
        print("graph_info This graph shows the number of node to node sessions of this coinevonet router")
        print("_peers_outbound.info Number of outbound coinevonet peers")
        print("_peers_inbound.info Number of inbound coinevonet peers")
        print("_peers_outbound.label outbound peers")
        print("_peers_inbound.label inbound peers")
        print("_peers_clients.info Number of coinevonet client peers")
        print("_peers_clients.label coinevonet client peers")
    else:
        inbound = Dict(int)
        outbound = Dict(int)
        clients = Dict(int)
...
if __name__ == '__main__':
    ...
    if exe == 'coinevonet_peers':
        peers_main()
    elif exe == 'coinevonet_exit':
        exit_sessions_main()
    else:
        print(
            'please symlink this as `coinevonet_peers` or `coinevonet_exit` in munin plugins dir')
