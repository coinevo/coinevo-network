...
namespace llarp
{
  ...
  KeyManager::initialize(const llarp::Config& config, bool genIfAbsent)
  {
    ...
    m_usingCoinevod       = config.coinevod.whitelistRouters;
    m_coinevodRPCAddr     = config.coinevod.coinevodRPCAddr;
    m_coinevodRPCUser     = config.coinevod.coinevodRPCUser;
    m_coinevodRPCPassword = config.coinevod.coinevodRPCPassword;
    ...
    {
      if(not loadIdentityFromCoinevod())
        return false;
    }
  ...
  bool
  KeyManager::loadIdentityFromCoinevod()
  {
    ...
    if(curl)
    {
      bool ret = false;
      std::stringstream ss;
      ss << "http://" << m_coinevodRPCAddr << "/json_rpc";
      const auto url = ss.str();
      curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
      curl_easy_setopt(curl, CURLOPT_HTTPAUTH, (long)CURLAUTH_ANY);
      const auto auth = m_coinevodRPCUser + ":" + m_coinevodRPCPassword;
      ...
      {
        resp.clear();
        LogInfo("Getting Identity Keys from coinevod...");
        if(curl_easy_perform(curl) == CURLE_OK)
        {
          try
          {
            ...
            if(k.size() != (identityKey.size() * 2))
            {
              if(k.empty())
              {
                LogError("coinevod gave no identity key");
              }
              else
              {
                LogError("coinevod gave invalid identity key");
              }
              return false;
            }
            ...
            {
              LogError("coinevod gave bogus identity key");
            }
          }
          catch(nlohmann::json::exception& ex)
          {
            LogError("Bad response from coinevod: ", ex.what());
          }
        }
        else
        {
        ...
        if(ret)
        {
          LogInfo("Got Identity Keys from coinevod: ",
