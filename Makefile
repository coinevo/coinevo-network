...
SHADOW_PLUGIN=$(BUILD_ROOT)/libshadow-plugin-coinevonet-shared.so
...
SHADOW_PLOT ?= $(PYTHON) $(SHADOW_SRC)/src/tools/plot-shadow.py -d $(REPO) CoinevoNET -c $(SHADOW_CONFIG) -r 10000 -e '.*'
SHADOW_OPTS ?=
...
COVERAGE_OUTDIR ?= "$(TMPDIR)/coinevonet-coverage"
...
TARGETS = $(REPO)/coinevonet
SIGS = $(TARGETS:=.sig)
EXE = $(BUILD_ROOT)/daemon/coinevonet
...
shadow-run: shadow-build
	...
	cp $(SHADOW_PLUGIN) $(REPO)/libshadow-plugin-coinevonet.so
...
android-gradle-prepare: $(LIBUV_PREFIX)
	...
	echo "coinevonetCMake=$(REPO)/CMakeLists.txt" >> $(ANDROID_PROPS)
...
windows-debug: windows-debug-configure
	$(MAKE) -C '$(BUILD_ROOT)'
	cp '$(BUILD_ROOT)/daemon/coinevonet.exe' '$(REPO)/coinevonet.exe'
...
ifeq ($(CLANG),OFF)
	gcovr -r . --branches --html --html-details -o "$(COVERAGE_OUTDIR)/coinevonet.html"
else
...
docker-kubernetes:
	docker build -f docker/coinevo-svc-kubernetes.Dockerfile .

install-pycoinevonet:
	cd $(REPO)/contrib/py/pycoinevonet && $(PYTHON3) setup.py install

kubernetes-install: install install-pycoinevonet
