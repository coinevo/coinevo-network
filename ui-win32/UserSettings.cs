...
namespace network.coinevo.coinevonet.win32.ui
{
    public partial class UserSettingsForm : Form
    {
        ...
        {
            try {
                Process.Start(string.Format("{0}/coinevonet.ini", config_path)); }
            catch
            {
        ...
        {
            if (File.Exists(string.Format("{0}/coinevonet.ini", config_path)))
            {
                DialogResult resp = MessageBox.Show("WARNING: This will overwrite your existing config file, Continue?", "Coinevonet", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                switch(resp)
                {
                    case DialogResult.Yes:
                        File.Delete(string.Format("{0}/coinevonet.ini", config_path));
                        break;
            ...
            p.StartInfo.FileName = coinevonetExeString;
        ...
        {
            MessageBox.Show(string.Format("Created new config file at {0}/coinevonet.ini", config_path), "Success", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }
    }
}
