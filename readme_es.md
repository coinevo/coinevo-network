...
Coinevonet es la implementación referente de LLARP (low latency anonymous routing protocol, protocolo de enrutado anónimo de baja latencia), un protocolo de enrutado onion de capa 3.
...
Puede ver la documentación, en ingles, de como empezar [aqui](https://coinevo.github.io/coinevo-docs/Coinevonet/CoinevonetOverview/) .
...
Vea, en ingles, [documentación](https://coinevo.github.io/coinevo-docs/Coinevonet/CoinevonetOverview/) en como comenzar.

También lea, en ingles, [La guia de pruebas publicas](https://coinevodocs.com/Coinevonet/Guides/PublicTestingGuide/#1-coinevonet-installation) para la instalación y mas información util.
...
El instalador de MAC coloca los binarios normales (`coinevonet` y `coinevonet-bootstrap`) en `/usr/local/bin` los que pudieran estar en ruta, asi que usted puede usar los binarios facilmente desde su terminal. El instalador tambien truena sus configuraciones y llaves previas, y descarga la semilla de arranque mas actual.
...
para correr como usuario, correr el archivo en lote `run-coinevonet.bat` como su usuario normal.
...
instalando (con priviligios elevados) en `$PROGRAMFILES/coinevonet` o `$ProgramFiles(x86)/coinevonet`:
